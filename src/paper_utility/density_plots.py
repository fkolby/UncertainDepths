...
def density_plot(idents:[str]) -> None:
    ...
    for i, ident in enumerate(idents):
        ...
        unc = torch.load(os.path.join(folder_location, ident, "_uncertainty_all_pixels.pt"), map_location=torch.device("cpu"))
        sca = torch.load(os.path.join(folder_location, ident, "scaled_uncertainty_all_pixels.pt"), map_location=torch.device("cpu"))
        OOD = torch.load(os.path.join(folder_location, ident, "OOD_class_all_pixels.pt"), map_location=torch.device("cpu"))
        ...
        plot = sns.histplot(data=random_data,x = "Scaled Uncertainty", hue="OOD class?", binwidth=0.02, stat="probability", common_norm=False)
        ...
        lst.append(pd.DataFrame(data = {"TPR":scaled_roc_curve_tpr, "FPR":scaled_roc_curve_fpr,"Model":f"{model}, AUROC: {roc_auc}"}))
    ...
    faceted_plot.map_dataframe(sns.histplot,x = "Scaled Uncertainty", hue="OOD class?", binwidth=0.01, stat="probability", common_norm=False, binrange=(0,0.6))
