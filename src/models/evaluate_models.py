    ...
    for sample in tqdm(test_loader, total=len(test_loader)):
        ...
        if config.save_images and i==0:
            for j in range(min(images.shape[0],6)):
                ...
                transforms.ToPILImage()(np.squeeze(d, axis=0)).save(os.path.join(config.save_images_path, f"{j}_{model_name}_depth.png"))
                transforms.ToPILImage()(np.squeeze(p, axis=0)).save(os.path.join(config.save_images_path, f"{j}_{model_name}_pred.png"))

                np.save(os.path.join(config.save_images_path, f"np_img_{model_name}_{j}.npy"),torch.squeeze(denormalize(image), dim=0).numpy(force=True))
                np.save(os.path.join(config.save_images_path, f"np_depth_{model_name}_{j}.npy"),torch.squeeze(depth, dim=0).numpy(force=True))
                np.save(os.path.join(config.save_images_path, f"np_preds_{model_name}_{j}.npy"),torch.squeeze(pred, dim=0).numpy(force=True))
