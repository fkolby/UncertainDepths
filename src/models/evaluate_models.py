...
def eval_model(
    ...
    for i, sample in enumerate(tqdm(test_loader, total=len(test_loader))):
        ...
        match cfg.models.model_type:
            ...
            case "Ensemble":
                ...
                for model_idx in range(cfg.models.n_models):
                    ...
                    preds[j, :, :, :, :] = torch.unsqueeze(
                        model(images), dim=0
                    ).to(device="cpu")  # unsqueeze to have a model-dimension (first)
                    j += 1
            case "Dropout":  # Only one model, but is different every call.
                ...
                for j in range(cfg.models.n_models):
                    preds[j, :, :, :, :] = torch.unsqueeze(
                        model(images), dim=0
                    ).to(device="cpu")  # unsqueeze to have a model-dimension (first) - move to cpu to free memory on gpu
        ...
        if cfg.save_images and i == 0:
            for j in range(min(images.shape[0], 6)):
                ...
                sd = torch.tensor(
                    np.transpose(
                        colorize(torch.squeeze(std_dev, dim=0), vmin=0, vmax=torch.quantile(std_dev,0.95)), (2, 0, 1)
                ...
                scaled_uncertainty = torch.tensor(
                    np.transpose(
                        colorize(torch.squeeze(scaled_uncertainty_uncolored, dim=0), vmin=0, vmax=torch.quantile(scaled_uncertainty_uncolored,0.95)), (2, 0, 1)
                    )  # None,None = v.min(), v.max() for color range
                ) 
                ...
                for key,value in {"depth":d,"pred":p,"sd":sd,"ScaledUncertainty":scaled_uncertainty}.items(): #save images/depths/etc to file
                ...
                for key, value in {"img":image,"depth":depth,"preds":pred,"std_dev":std_dev,"ScaledUncertainty":scaled_uncertainty_uncolored}.items():
                    if key=="img":
                        np.save(
                            os.path.join(
                                cfg.save_images_path,
        ...
        if cfg.OOD.use_white_noise_box_test:
            ...
            _, _, uncertainty_whitenoise = filter_valid(gt=depths, pred=OOD_class, uncertainty=uncertainty, config=cfg) #hacky way of obtaining valid pixels for OOD-class - we only filter on depths (ground truth)
            _, OOD_class, scaled_uncertainty_whitenoise = filter_valid(gt=depths, pred=OOD_class, uncertainty=scaled_uncertainty_uncolored, config=cfg) 
            
            #aggregate white noise estimates.
            uncertainty_in_distribution = torch.cat([ uncertainty_in_distribution,uncertainty_whitenoise[OOD_class==0].flatten()])
            uncertainty_out_of_distribution = torch.cat([ uncertainty_out_of_distribution,uncertainty_whitenoise[OOD_class==1].flatten()])
            scaled_uncertainty_in_distribution = torch.cat([ scaled_uncertainty_in_distribution,scaled_uncertainty_whitenoise[OOD_class==0].flatten()])
            scaled_uncertainty_out_of_distribution = torch.cat([ scaled_uncertainty_out_of_distribution,scaled_uncertainty_whitenoise[OOD_class==1].flatten()])
    ...
    if cfg.OOD.use_white_noise_box_test: #log it.
        ...
        wandb.log({"uncertainty In distribution:": uncertainty_in_distribution.mean(),
              "uncertainty out of distribution": uncertainty_out_of_distribution.mean(),
              "scaled uncertainty in distribution": scaled_uncertainty_in_distribution.mean(), 
              "scaled uncertainty out of distribution": scaled_uncertainty_out_of_distribution.mean()})
    ...
    print("sum of absolute differences in ranking:",torch.sum(torch.abs(torch.tensor(sort_by_scaled_uncertainty_ascending_indices)-torch.tensor(sort_by_uncertainty_ascending_indices))))
    print("count of absolute differences in ranking:",torch.sum(torch.abs(torch.tensor(sort_by_scaled_uncertainty_ascending_indices)-torch.tensor(sort_by_uncertainty_ascending_indices))>0))
