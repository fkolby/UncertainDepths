...
def img_dicts():
    ...
    for el in visuals:
        ...
        if im_type=="img": #Resize, so preds/depths can be color-corrected
            d[model][im_type] = d[model][im_type] + [Resize((352,704))(torch.tensor(np.load(os.path.join(path_to_pics, el)))).numpy(force=True)]
        elif im_type=="depth": #Resize, so preds/depths can be color-corrected
            d[model][im_type] = d[model][im_type] + [Resize((352,704))(torch.unsqueeze(torch.tensor(np.load(os.path.join(path_to_pics, el))),dim=0)).squeeze().numpy(force=True)]
        else:
            d[model][im_type] = d[model][im_type] + [Resize((352,704))(torch.unsqueeze(torch.tensor(np.load(os.path.join(path_to_pics, el))),dim=0)).squeeze().numpy(force=True)]
    return d
...
def main():
    ...
    with server.add_gui_folder("Editable"):
        ...
        campos = server.add_gui_vector3(
            "Campos",
            initial_value=(37.,39.,-6.),
            step=0.1,
        )
        with server.add_gui_folder("Text toggle"):
            img_type = server.add_gui_dropdown(
        "Img type plane", ("preds", "depth", "img")
    )
            model = server.add_gui_dropdown(
                "model",
                ("ZoeNK","Unet","ZoeNK"))
    ...
    for y in range(depth.shape[0]):
        for x in range(depth.shape[1]):
            ...
            colors[y*depth.shape[1]+x,:] = img[:,y,x]*255 #((80-depth[y,x])/80)**2*np.array([255,0,0])
        ...
        if img_type.value != "img":
            ...
            for y in range(depth.shape[1]):
                for x in range(depth.shape[2]):
                    ...
                    point_positions[y*depth.shape[2]+x,2] = (0)    #color_coeffs = np.random.uniform(0.4, 1.0, size=(point_positions.shape[0]))
                    colors[y*depth.shape[2]+x,:] = img[:,y,x]*255#((80-depth[y,x])/80)**2*np.array([255,0,0])
